  ...
  angular
    ...
    .directive('menuToggle', ['$mdUtil', '$animateCss', '$$rAF', function($mdUtil, $animateCss, $$rAF) {
      return {
        ...
        templateUrl: 'modules/core/client/views/templates/menu-toggle.client.tmpl.html',
        link: function($scope, $element) {
          var controller = $element.parent().controller();

          // Used for toggling the visibility of the accordion's content after all of the animation are completed
          // This prevent users from being allowed to tab through to the hidden content.
              ...
              $$rAF(function () {
                ...
                $animateCss($ul, {
                  ...
                  if (open && $li && $ul[0].scrollTop === 0) {
                    ...
                    // Reduce a bit (2 list items height worth) so it doesn't touch the nav
                    var negativeOffset = activeHeight * 2;
                    var newScrollTop = (activeOffset + parentScrollPosition) - negativeOffset;

                    $mdUtil.animateScrollTo(document.querySelector('.bds-menu').parentNode, newScrollTop);
                  }
                });
              });
            });
